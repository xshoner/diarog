...
## 0.2.1: 한국어 모델 TIME OUT 해결

- **모델 내장 APK**는 `한국어 모델 설치 · 인터넷 불필요`를 누르면 앱에 포함된 원본 모델을 복사하고 SHA-256/압축/음성 엔진을 검증합니다. 비행기 모드에서도 설치할 수 있으며 별도 네트워크 다운로드가 없습니다. 설치 중에는 여유 공간 400MB가 필요합니다.
- 일반 APK는 연결 30초, 읽기 60초의 제한을 명시하며 중단된 파일을 앱 전용 비백업 공간에 보관합니다. 다음 작업은 HTTP Range로 이어받습니다. 200 응답은 처음부터 덮어쓰고, 잘못된 206 범위와 손상된 파일은 거절합니다. 작업당 네트워크 처리 예산은 약 3분이고 이후 작업에서 이어받습니다.
- 공식 모델 서버의 Range 응답(206)과 모델 ZIP SHA-256을 확인했습니다. 타임아웃 후 이어받기, Range 무시, 잘못된 범위, 완성 파일의 오프라인 재사용, 작업 예산 중단을 자동 테스트합니다.
- 모델 내장 배포는 일반 APK보다 약 83MB 큽니다. 기존 APK와 같은 서명으로 빌드해야 설정을 유지하며 업데이트할 수 있습니다.

모델 내장 APK 빌드: `vosk-model-small-ko-0.22.zip` 원본을 별도 디렉터리에 넣고, SHA-256이 `eea36124087fed26c59996a4761519458e3bd185e8ea9d9865ad8760c4a1d989`인지 확인한 뒤 `./gradlew -PbundledModelDir=/absolute/model-directory :app:assembleDebug`를 실행합니다. 모델 ZIP은 저장소에 커밋하지 않습니다. APK 내부 `assets/vosk-model-small-ko-0.22.zip`의 해시도 동일해야 합니다. 모델 라이선스 고지는 앱에 포함되어 있습니다.
