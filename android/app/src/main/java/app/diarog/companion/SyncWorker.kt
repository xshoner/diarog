        ...
        SyncGate.mutex.withLock {
            ...
            try {
                ...
                QueueStore(applicationContext).use { queue ->
                    ...
                    catch (_: Exception) { settings.status = "녹음 폴더를 읽지 못했습니다. 폴더를 다시 선택해 주세요." }
                    ...
                    for(item in queue.pending()) {
                        ...
                        try { api.upload(item); queue.acknowledge(item.id) }
                        catch (e: HttpFailure) {
                            if(e.code == 401 || e.code == 403) throw e
                            queue.fail(item, e.code)
                            transientFailure = true
                            if(e.code == 429 || e.code >= 500) break
                        } catch (_: java.io.IOException) { queue.fail(item, 0); transientFailure = true; break }
            ...
            } catch (_: Exception) { settings.status = "동기화 실패. 네트워크와 연결 설정을 확인해 주세요."; Result.retry() }
        }
    }
