...
try {
  ...
  assert.ok(queries.filter(q => ["personal_memories", "persona_edits", "diary_entries"].some(table => q.url.pathname.endsWith(table))).every(q => q.url.searchParams.get("user_id") === "eq.fixture-owner"));
  console.log("PASS: Next.js persona page/API, session auth, same-origin protection and owner-scoped reads/deletes");
} finally {
  child.kill();
